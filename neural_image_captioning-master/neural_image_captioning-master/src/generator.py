...
class Generator():
    ...
    def __init__(self, data_path='preprocessed_data/',
        ...
        if training_filename == None:
            self.training_filename = os.path.join(data_path, 'training_data.txt')
        else:
            self.training_filename = os.path.join(self.data_path, training_filename)

        if validation_filename is None:
            self.validation_filename = os.path.join(data_path, 'validation_data.txt')
        else:
            self.validation_filename = os.path.join(self.data_path, validation_filename)

        if image_features_filename is None:
            self.image_features_filename = os.path.join(data_path, 'inception_image_name_to_features.h5')
        else:
            self.image_features_filename = os.path.join(self.data, image_features_filename)
    ...
    def load_vocabulary(self):
        ...
        word_to_id = pickle.load(open(os.path.join(self.data_path, 'word_to_id.p'), 'rb'))
        id_to_word = pickle.load(open(os.path.join(self.data_path, 'id_to_word.p'), 'rb'))
