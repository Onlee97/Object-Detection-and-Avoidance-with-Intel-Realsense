...
class Evaluator(object):
    ...
    def __init__(self, model,
        ...
        self.test_data = pd.read_table(os.path.join(data_path, test_data_filename), sep='*')
        self.word_to_id = pickle.load(open(os.path.join(data_path, word_to_id_filename), 'rb'))
        self.id_to_word = pickle.load(open(os.path.join(data_path, id_to_word_filename), 'rb'))
        self.VOCABULARY_SIZE = len(self.word_to_id)
        self.image_names_to_features = h5py.File(os.path.join(data_path, image_name_to_features_filename))

    def _load_log_file(self):
        data_logs = np.genfromtxt(os.path.join(self.data_path, 'data_parameters.log'),
                                  delimiter=' ', dtype='str')
    ...
    def display_caption(self, image_file=None, data_name=None):
        ...
        print(self.images_path, image_name)
        plt.imshow(plt.imread(os.path.join(self.images_path, image_name)))
        plt.show()

    def write_captions(self, dump_filename=None):
        if dump_filename == None:
            dump_filename = os.path.join(self.data_path, 'predicted_captions.txt')
        ...
        target_captions.to_csv(os.path.join(self.data_path + 'target_captions.txt'), header=False, index=False)
